  ...
  const handleCompletePurchase = async () => {
    ...
    try {
      const results = await Promise.all(
        cartItems.map((item) =>
          checkItemAvailability(item.productId, item.quantity),
        ),
      );
...
const CartItemComponent: React.FunctionComponent<{
  item: CartItemData;
  onDelete: () => void;
}> = ({ item, onDelete }) => {
  return (
    <ListGroup.Item className={styles.cartItem}>
      <Row>
        <Col>
          <img
            src={item.productImage || "../../../defaultproduct.png"}
            className={styles.imgProductoCart}
