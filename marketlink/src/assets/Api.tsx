...
export const getUsers = async (): Promise<User[]> => {
  ...
  const users = querySnapshot.docs.map(
    (doc) => new User(doc.id, doc.data().email, doc.data().name),
  );
...
export const getCartItems = async (userId: string): Promise<CartItemData[]> => {
  ...
  const q = query(
    ...
    where("cartId", "==", doc(db, "Cart", cart.id)),
  );
...
export const getProductsByEntrepreneur = async (
...
export const getEntrepreneurByCartItemId = async (
  cartItemId: string,
): Promise<Entrepreneur> => {
...
export const deleteCartItems = async (userId: string): Promise<void> => {
  ...
  const q = query(
    collection(db, "CartItem"),
    where("cartId", "==", doc(db, "Cart", cartId.id)),
  );
  const querySnapshot = await getDocs(q);
...
export const getOrders = async (userId: string): Promise<Order[]> => {
  ...
  const orders = await Promise.all(
    transactionsSnapshot.docs.map(async (transactionDoc) => {
      ...
      const transactionItemsQuery = query(
        collection(db, "TransactionItem"),
        where("transaction", "==", doc(db, "Transaction", transactionDoc.id)),
      );
      ...
      const items = await Promise.all(
        transactionItemsSnapshot.docs.map(async (itemDoc) => {
...
export const updateClientUserName = async (
  userId: string,
...
export const getEntrepreneurOrders = async (
  entrepreneurId: string,
): Promise<EntrepreneurOrder[]> => {
  const productRef = query(
    collection(db, "Product"),
    where("entrepreneur", "==", doc(db, "Entrepreneur", entrepreneurId)),
  );
  const productSnapshot = await getDocs(productRef);
  const orders = [];

  for (const product of productSnapshot.docs) {
    const transactionItemRef = query(
      collection(db, "TransactionItem"),
      where("product", "==", doc(db, "Product", product.id)),
    );
...
export const updateEntrepreneurOrderStatus = async (
  orderId: string,
  newStatus: string,
): Promise<void> => {
  try {
